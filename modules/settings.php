...
// M-2: SMS balance is now fetched asynchronously via JS calling api/sms_balance.php
// Removed synchronous fetch that blocked page render for up to 30 seconds
    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-200">
        ...
        <div class="p-8">
                        ...
                        <div id="spellcpaasApiKeySection" class="<?= $selectedSmsProvider === 'spellcpaas' ? '' : 'hidden' ?>">
                            ...
                            <div class="space-y-2">
                                ...
                                <input 
                                    ...
                                    value=""
                                    placeholder="<?= $smsApiKey !== '' ? '••••••••' . substr($smsApiKey, -4) . ' (leave blank to keep current)' : 'Enter API key' ?>"
                                    <?= $selectedSmsProvider === 'spellcpaas' && $smsApiKey === '' ? 'required' : '' ?>>
                                <p class="text-xs text-slate-500 mt-1.5">Leave blank to keep the current key. Only saved if you enter a new value.</p>
                            </div>
                        </div>

                        <?php if($selectedSmsProvider === 'spellcpaas'): ?>
                            <div class="border-t border-slate-200 pt-6">
                                <h3 class="text-sm font-semibold text-slate-900 mb-4 uppercase tracking-wider">Credit Balance</h3>
                                <?php if($smsApiKey === ''): ?>
                                    <p class="text-sm text-slate-500">Save your API key first to check balance.</p>
                                <?php else: ?>
                                    <div class="flex items-center gap-3">
                                        <div id="smsBalanceDisplay" class="rounded-lg border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-600">Click "Check Balance" to load</div>
                                        <button type="button" id="checkBalanceBtn" class="inline-flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-medium bg-slate-100 hover:bg-slate-200 border border-slate-200 transition-colors">
                                    ...
                                    document.getElementById('checkBalanceBtn').addEventListener('click', function(){
                                        var btn = this;
                                        var display = document.getElementById('smsBalanceDisplay');
                                        btn.disabled = true;
                                        display.textContent = 'Loading...';
                                        fetch('<?= e(get_base_url()) ?>/api/sms_balance.php', {credentials:'same-origin'})
                                            .then(function(r){ return r.json(); })
                                            .then(function(d){
                                                if(d.success){
                                                    display.textContent = 'Credits: ' + (d.balance || 0);
                                                    display.className = 'rounded-lg border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-700 font-semibold';
                                                } else {
                                                    display.textContent = d.message || 'Error';
                                                    display.className = 'rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700';
                                                }
                                            })
                                            .catch(function(){ display.textContent = 'Network error'; })
            ...
            <div id="settingsTabLicense" class="settings-tab-panel" style="display:none;">
                ...
                <!-- Status Banner -->
                <?php
                $licValid = !empty($licenseStatus['valid']);
                $licPlan  = !empty($licenseStatus['plan']) ? ucfirst((string)$licenseStatus['plan']) : '';
                $licMsg   = (string)($licenseStatus['message'] ?? 'No license key configured.');
                $licExp   = !empty($licenseStatus['expires_at']) ? date('d M Y', strtotime((string)$licenseStatus['expires_at'])) : '';
                ?>
                <div class="rounded-xl border-2 px-6 py-5 mb-8 <?= $licValid ? 'bg-emerald-50 border-emerald-200' : 'bg-red-50 border-red-200' ?>">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-3">
                            <div class="w-3 h-3 rounded-full flex-shrink-0 <?= $licValid ? 'bg-emerald-500' : 'bg-red-500' ?>"></div>
                            <div>
                                <p class="font-bold text-base <?= $licValid ? 'text-emerald-800' : 'text-red-800' ?>">
                                    <?= $licValid ? 'License Active' : 'License Invalid' ?>
                                </p>
                                <?php if($licPlan !== ''): ?>
                                    <p class="text-sm <?= $licValid ? 'text-emerald-600' : 'text-red-600' ?>">Plan: <?= e($licPlan) ?></p>
                                <?php else: ?>
                                    <p class="text-sm <?= $licValid ? 'text-emerald-600' : 'text-red-600' ?>"><?= e($licMsg) ?></p>
                                <?php endif; ?>
                                <?php if($licExp !== ''): ?>
                                    <p class="text-xs mt-1 <?= $licValid ? 'text-emerald-500' : 'text-red-500' ?>">Expires: <?= e($licExp) ?></p>
                                <?php endif; ?>
                            </div>
                        </div>
                        <form method="POST">
                            ...
                            <button type="submit" class="inline-flex items-center gap-1.5 px-4 py-2 rounded-lg text-xs font-medium bg-white hover:bg-slate-50 border border-slate-300 transition-colors shadow-sm">
                ...
                <!-- Current License Key -->
                <div class="mb-8">
                    <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-3">Current License Key</label>
                    <div class="px-5 py-4 bg-slate-50 border border-slate-200 rounded-xl font-mono text-sm text-slate-700">
                        <?= $licenseMasked !== '' ? e($licenseMasked) : '<span class="text-slate-400 italic" style="font-family:inherit">No key configured</span>' ?>
                    </div>
                </div>

                <!-- Update License Key -->
                <div class="border-t border-slate-200 pt-8">
                    <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-3">Update License Key</label>
                    <form method="POST">
                        <input type="hidden" name="csrf" value="<?= csrf_token() ?>">
                        ...
                        <div class="flex gap-4">
                            <input
                                ...
                                class="flex-1 px-4 py-3 bg-white border border-slate-300 rounded-xl text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent transition-all font-mono text-sm"
                                oninput="this.value=this.value.toUpperCase()"
                            >
                            <button type="submit" class="inline-flex items-center gap-2 bg-primary hover:bg-teal-800 text-white px-6 py-3 rounded-xl text-sm font-semibold transition-colors shadow-sm flex-shrink-0">
                                Activate
                            </button>
                        </div>
                        <p class="text-xs text-slate-400 mt-3">Enter your PharmaCore license key to activate the product.</p>
                    </form>
                </div>
            </div>
            <!-- ── /License Tab Panel ────────────────────────────────────── -->
