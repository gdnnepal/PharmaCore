...
<div class="flex h-screen overflow-hidden">
    ...
    <div class="flex-1 flex flex-col overflow-hidden">
        ...
        <main class="flex-1 overflow-y-auto p-5">
            <?php
            // Consume flash message ONCE here — rendered as toast via JS below
            $__flash = flash_msg();
            include $module_file;
            ?>
        </main>
...
window.__showToast = function(message, type){
    ...
    toast.style.cssText = 'pointer-events:auto;display:flex;align-items:flex-start;gap:12px;padding:12px 16px;border-radius:12px;box-shadow:0 10px 25px rgba(0,0,0,0.1);border:1px solid;font-size:14px;transition:all 0.3s ease;transform:translateX(100%);opacity:0;background:#fff;max-width:380px;';
    toast.style.borderColor = type === 'error' ? '#fecaca' : '#a7f3d0';
    toast.style.color = type === 'error' ? '#991b1b' : '#065f46';

    var icon = type === 'error'
        ? '<svg style="width:16px;height:16px;margin-top:2px;flex-shrink:0;color:#ef4444" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clip-rule="evenodd"/></svg>'
        : '<svg style="width:16px;height:16px;margin-top:2px;flex-shrink:0;color:#10b981" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/></svg>';

    toast.innerHTML = icon + '<span style="flex:1">' + message + '</span>'
        + '<button onclick="this.parentElement.remove()" style="margin-left:8px;color:#94a3b8;cursor:pointer;border:none;background:none;font-size:18px;line-height:1">&times;</button>';
...
<div id="toastContainer" style="position:fixed;top:16px;right:16px;z-index:9999;display:flex;flex-direction:column;gap:8px;pointer-events:none;max-width:380px;"></div>
<?php if($__flash): ?>
<script>
(function(){
    var type = <?= json_encode($__flash['type'] ?? 'success', JSON_HEX_TAG) ?>;
    var msg = <?= json_encode($__flash['msg'] ?? '', JSON_HEX_TAG | JSON_HEX_AMP) ?>;
    if(msg) window.__showToast && window.__showToast(msg, type);
})();
</script>
<?php endif; ?>
</body></html>